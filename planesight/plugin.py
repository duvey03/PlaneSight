...
Responsible only for lifecycle (initGui/unload) and launching the UI. All
...
from .gui.main_dialog import PlaneSightDialog
...
class PlaneSightPlugin:
    ...
    def unload(self):
        """Remove the action (called by QGIS on unload)."""
    ...
    def run(self):
        """Open the main PlaneSight dialog."""
        if self._dialog is None:
            self._dialog = PlaneSightDialog(self.iface.mainWindow())
        self._dialog.show()
        self._dialog.raise_()
